...
$id = $_GET['invoice_id'] ?? $_POST['invoice_id'] ?? 0;
...
if($checkPaid['status'] == "Paid"){
    ...
    window.location='dashboard.php?page=invoice_system/payments/payment_list.php';
    </script>";
    exit;
}
...
SELECT 
    ...
    students.email, 
    students.first_name,
    enrollment_inquiries.mode_of_education
FROM students
JOIN invoices ON invoices.student_id = students.id
LEFT JOIN enrollment_inquiries 
    ON enrollment_inquiries.student_id = students.id
WHERE invoices.id = '$invoice_id'
"));

if($studentData && $studentData['mode_of_education'] != "Offline"){
    ...
    // 🔗 create reset link
    $reset_link = $baseUrl."/Student_dashboard/auth/set_password.php?token=".$token;
...
history.pushState(null, '', '?page=invoice_system/payments/payment_list.php');

$.get('invoice_system/payments/payment_list.php', function(data){
